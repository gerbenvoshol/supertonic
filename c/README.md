...
- **Voice Builder**: Create custom voice styles from WAV files (experimental)
- **Voice Mixer**: Blend existing high-quality voice styles to create custom voices
...
- `voice_builder.c` - Tool for creating custom voice styles from WAV files (experimental)
- `voice_mixer.c` - Tool for blending existing voice styles to create custom voices
...
This will compile all source files and create four executables: `example_onnx`, `audiobook_generator`, `voice_builder`, and `voice_mixer`.
...
✨ **HIGH-QUALITY VOICE BLENDING TOOL**

The `voice_mixer` tool creates custom voice personalities by **blending existing high-quality voice styles**. Unlike `voice_builder` which generates pseudo-random styles, this tool interpolates between pre-trained voice styles, ensuring high-quality output.

### Key Advantages

- ✅ **High Quality**: Interpolates between official voice styles, producing natural speech
- ✅ **Immediate Results**: No training or encoding required
- ✅ **Flexible Blending**: Combine 2+ voices with custom weights
- ✅ **Advanced Modes**: Supports both linear and spherical (slerp) interpolation
- ✅ **Creative Exploration**: Random blend mode for discovering new voices

### How It Works

Voice styles consist of two neural network embedding vectors:
- **style_ttl**: Text-to-latent features [1, 50, 256] = 12,800 floats
- **style_dp**: Duration predictor features [1, 8, 16] = 128 floats

The voice mixer performs weighted interpolation of these vectors to create new voices that blend characteristics of the source voices.

### Quick Start

```bash
# Blend two voices equally (50/50 mix)
./voice_mixer --voices ../assets/voice_styles/M1.json,../assets/voice_styles/F1.json \
  --output mixed.json

# Weighted blend: 70% male, 30% female
./voice_mixer --voices ../assets/voice_styles/M1.json,../assets/voice_styles/F1.json \
  --weights 0.7,0.3 --output mixed.json
...
**Basic Options:**
- `--voices <paths>` - **Required** (unless --random): Comma-separated paths to voice style JSON files
- `--weights <values>` - Comma-separated blend weights (default: equal weights)
- `--output <file>` - Output JSON file (default: mixed_voice.json)
- `--mode <type>` - Blend mode: `linear` or `slerp` (default: linear)
- `--help, -h` - Show help message

**Advanced Options:**
- `--random` - Generate random blend from available voices
- `--num-random <n>` - Number of voices for random blend (default: 2)
- `--voice-dir <path>` - Directory with voice styles (default: ../assets/voice_styles)
- `--list-voices` - List available voices in voice-dir

### Blend Modes

**Linear Interpolation (default)**
- Standard weighted average: `result = w1*v1 + w2*v2 + ... + wn*vn`
- Fast and simple
- Good for most use cases

**Spherical Interpolation (slerp)**
- Treats style vectors as points on a hypersphere
- Interpolates along the surface of the sphere
- Often produces smoother, more natural-sounding blends
- Recommended for perceptually important features
...
**Weighted blend (70% M1, 30% F1):**
```bash
./voice_mixer --voices ../assets/voice_styles/M1.json,../assets/voice_styles/F1.json \
  --weights 0.7,0.3 --output voice_70_30.json
```

**Blend three voices:**
```bash
./voice_mixer --voices M1.json,F1.json,M3.json \
  --weights 0.5,0.3,0.2 --output voice_three.json
```

**Spherical interpolation mode:**
```bash
./voice_mixer --voices ../assets/voice_styles/M1.json,../assets/voice_styles/F1.json \
  --weights 0.7,0.3 --mode slerp --output voice_slerp.json
```

**Random blend for creative exploration:**
```bash
# Random blend of 2 voices
./voice_mixer --random --voice-dir ../assets/voice_styles \
  --output random_voice.json

# Random blend of 3 voices
./voice_mixer --random --num-random 3 --voice-dir ../assets/voice_styles \
  --output random_3voice.json
```

**List available voices:**
```bash
./voice_mixer --list-voices --voice-dir ../assets/voice_styles
```

### Testing Blended Voices

After creating a blended voice, test it with `example_onnx`:

```bash
# Test the blended voice
./example_onnx --voice-style mixed.json --text "Hello, this is my custom blended voice!"

# Generate audiobook with blended voice
./audiobook_generator --input story.txt --voice mixed.json --output story_mixed.wav
```

### Tips for Best Results

1. **Start Simple**: Begin with 50/50 blends to understand how voices combine
2. **Experiment with Weights**: Try different weight ratios (e.g., 0.8/0.2, 0.6/0.4)
3. **Try Slerp Mode**: For some voice combinations, slerp produces more natural results
4. **Blend Similar Voices**: Mixing voices of similar quality/style often works better
5. **Use Random Mode**: Great for discovering unexpected voice combinations
6. **Test Thoroughly**: Try your blended voice with different texts and languages

### Advanced Techniques

**Creating Voice Variations:**
```bash
# Create multiple variations from the same voices
./voice_mixer --voices M1.json,F1.json --weights 0.9,0.1 --output voice_90.json
./voice_mixer --voices M1.json,F1.json --weights 0.7,0.3 --output voice_70.json
./voice_mixer --voices M1.json,F1.json --weights 0.5,0.5 --output voice_50.json
./voice_mixer --voices M1.json,F1.json --weights 0.3,0.7 --output voice_30.json
```

**Multi-voice Blending:**
```bash
# Blend 4 voices for unique characteristics
./voice_mixer \
  --voices M1.json,F1.json,M3.json,F5.json \
  --weights 0.4,0.3,0.2,0.1 \
  --output voice_complex.json
```

### Output Format

The generated JSON file is fully compatible with all Supertonic TTS implementations. It can be used anywhere a standard voice style JSON is expected:
- C implementation (this directory)
- Python implementation (`py/`)
- Node.js implementation (`nodejs/`)
- All other Supertonic TTS implementations
...
- Input: `--weights 2,3` → Normalized: `0.4, 0.6`
- Input: `--weights 10,5,5` → Normalized: `0.5, 0.25, 0.25`

**Spherical Interpolation (SLERP):**
For two vectors, slerp computes: `result = sin((1-t)θ)/sin(θ) * v1 + sin(tθ)/sin(θ) * v2`
where θ is the angle between vectors and t is the interpolation parameter.

For multiple voices (>2), sequential slerp is performed: `((v1 slerp v2) slerp v3) slerp v4...`

**Why This Works:**
Unlike voice_builder's random features, voice_mixer blends actual voice embeddings that were created by trained encoder models. This preserves the semantic meaning of the embedding space, resulting in high-quality output.
